...
class Command(BaseCommand):
    help = 'Send invoice emails for orders that need them'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--order-id',
            type=int,
            help='Send invoice email for specific order ID',
        )
        ...
        parser.add_argument(
            '--status',
            choices=['confirmed', 'shipped', 'delivered', 'all'],
            default='all',
            help='Only process orders with specific status (default: all eligible)',
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=30,
            help='Maximum age of orders to process in days (default: 30)',
        )
    ...
    def _process_single_order(self, order, email_service, options, stats):
        ...
        if options['dry_run']:
            self.stdout.write(
                self.style.WARNING(f'DRY RUN: Would send invoice email for order {order.order_number}')
            )
            stats['successful'] = 1
            return
        ...
        if success:
            stats['successful'] = 1
            self.stdout.write(
                self.style.SUCCESS(f'✓ Sent invoice email for order {order.order_number}')
            )
        else:
            stats['failed'] = 1
            stats['errors'].append(f"Order {order.order_number}: {message}")
            self.stdout.write(
                self.style.ERROR(f'✗ Failed to send invoice for order {order.order_number}: {message}')
            )
    ...
    def _process_bulk_orders(self, email_service, options, stats):
        ...
        # Status filter - only send invoices for eligible orders
        if options['status'] == 'all':
            query &= Q(status__in=['confirmed', 'shipped', 'delivered'])
        else:
            query &= Q(status=options['status'])
        
        # Only process orders that need invoice emails
        if not options['force']:
            query &= Q(invoice_email_sent=False)
        
        # Get orders
        orders = Order.objects.filter(query).select_related('user')[:options['limit']]
        
        if not orders:
            self.stdout.write(self.style.SUCCESS('No orders found that need invoice emails.'))
            return
        ...
        for order in orders:
            ...
            if order.invoice_email_sent and not options['force']:
                stats['skipped'] += 1
                continue
            
            # Skip if order status is not eligible for invoices
            if order.status not in ['confirmed', 'shipped', 'delivered'] and options['status'] == 'all':
                stats['skipped'] += 1
                continue
